...
func TestClientGettersAndFailures(t *testing.T) {
	...
	if err != nil || resp.Success {
		t.Fatal("expected no err but false success for invalid profile")
	}

	_, err = client.CreateAndInscribeProfile(ctx, HCS11Profile{}, false, InscribeProfileOptions{})
	if err != nil {
		t.Fatal("expected no generic error from wrapper")
	}
}
...
func TestAssignProfileOptions(t *testing.T) {
	...
	if profile.Alias != "my-alias" {
		t.Fatal("expected alias")
	}
	if profile.Bio != "my bio" {
		t.Fatal("expected bio")
	}
	if profile.ProfileImage != "img.png" {
		t.Fatal("expected img")
	}
	if profile.InboundTopicID != "0.0.1" {
		t.Fatal("expected inbound")
	}
	if profile.OutboundTopicID != "0.0.2" {
		t.Fatal("expected outbound")
	}
	if profile.BaseAccount != "0.0.3" {
...
func TestValidateProfileBranches(t *testing.T) {
	...
	if result.Valid {
		t.Fatal("expected invalid flora")
	}
	...
	if result2.Valid {
		t.Fatal("expected invalid transport")
	}
